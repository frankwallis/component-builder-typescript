...
function bundle(options) {
	...
	var inputFiles = optValue(options.inputFiles, []);
	...
	var sourceTypings = Object.keys(inputFiles)
		.filter(function(file) {
			return Compiler.isTypescriptDeclaration(file);
		})
		.map(function(file) {
			return path.resolve(baseDir, file);
		});

	assert(sourceTypings.indexOf(mainFile) >= 0, 'main does not exist: ' + mainFile);
	...
	function parseFile(file) {
		...
		var code = inputFiles[cleanupName(file)];

		if (!code )
			console.log('dts-bundle: file not found [' + file + ']');
	...
	content += '// Generated by dts-bundle' + newline;
