...
function Compiler(opts) {
	...
	if (opts.externalDeclaration) {
		this.opts.declaration = true;
	}
...
Compiler.prototype.getCompiledFile = function (branch, tsFile) {
	...
	//console.log('b.path' + branch.path);
	...
	//var normalized = ts.normalizePath(outputPath);

	var output = self.host.cachedOutput[outputPath];

	if (path.basename(inputPath) == 'authentication-events.ts') {
			console.log(JSON.stringify(Object.keys(self.host.cachedOutput)));
