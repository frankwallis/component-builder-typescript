...
//var tsd  = require('./declarations-sync');
...
function Compiler(opts) {
	...
	this.ignoreFilesPattern = opts.ignoreFilesPattern;
...
Compiler.prototype.shouldIgnoreFile = function (path, branch) {
	return this.ignoreFilesPattern && (branch.type != 'local') && this.ignoreFilesPattern.test(path);
}
...
Compiler.prototype.updateFile = function (file, cmpout) {
	...
	if (!file.string) {
		...
		cmpout.error('typescript', file.branch.name + '/' + file.path + ' has not been compiled');
		return false;
	}
	else {
		file.extension = 'js';
