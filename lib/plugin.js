...
module.exports = function(options) {
  ...
  options = options || { compileLinkedComponents: false };  
  ...
  return function(file, done) {
    ...
    if (!file.branch.__tsc) {
      ...
      initialiseBranch(file.branch, file.manifest, function(err) {
  ...
  function initialiseBranch(branch, manifest, done) {
    ...
    async.series([
        ...
        function(callback) {
          if (isLocal && hasTypeScript)
            outputDeclarations(branch, manifest, callback)
          else 
            callback();
        },
      ], done);
  ...
  function compileBranch(branch, manifest, isLocal) {
    ...
    var tsSource = manifest.field.scripts
      .filter(function(script) {            
        return path.extname(script.path) == '.ts';
      })
      .map(function(script) {
