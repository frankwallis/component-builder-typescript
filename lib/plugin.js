...
function compileBranch(branch, manifest) {
  if (!branch.__tsc) {
    ...
    var isLocal = (branch.type == 'local'); //stats.isSymbolicLink() || (branch.type == 'local');
    ...
    if (tsscripts.length > 0) {
      ...
      var result = tsapi.compileWithHost(host, tsscripts, cmdargs, settings, handleErr);
...
function compileJsFile(file) {
  // TODO - check what happens if there is a js file accidentally picked up by
  // component
