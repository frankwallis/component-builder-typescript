...
module.exports = function(options) {
  ...
  options = options || { compileLinkedComponents: true };  

  return function(file, done) {
    initialiseBranch(file.branch, file.manifest, function() {
  ...
  function initialiseBranch(branch, manifest, done) {
    if (!branch.__tsc) {
      branch.__tsc = {
        jsFileCache: {},
        tsFileCache: {},
      ...
      async.series([
          function(callback) {
            ...
            if (options.compileLinkedComponents) {
              fs.lstat(branch.path, function(err, stats) {
                isLocal = isLocal || stats && stats.isSymbolicLink();
                callback();
              });
            }
            else {
          ...
          function(callback) {
            if (isLocal)
              outputDependencies(branch, manifest, callback);
            else 
              callback();
          },
          function(callback) {
            compileBranch(branch, manifest, isLocal);
            updateFileCache(branch);
            callback();
          },
          function(callback) {
            if (isLocal)
              outputDeclarations(branch, manifest, done)
            else 
              callback();
          },
        ], done);
  ...
  function compileBranch(branch, manifest, isLocal) {
    ...
    if (tsSource.length > 0) {
      ...
      var cmdargs = '-m commonjs -t es5';

      // output to /tmp as we need to write the declaration files for dts-bundle
      if (isLocal)
        cmdargs = cmdargs + ' -d -outDir ' + path.join(branch.path, 'tmp');
  ...
  function updateFileCache(branch) {
    Object.keys(branch.__tsc.tsFileCache).forEach(function(key) {
      ...
      if (!file.string) {
        ...
        console.log('populating ' + key + ' with ' + file.string);
      }
    });
  }
