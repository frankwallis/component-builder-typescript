...
function BranchCompiler(opts) {
	this.opts = {
		...
		generateSourceMaps: opts.generateSourceMaps || false,
		generateDeclarations: opts.generateDeclarations || true,
		target: opts.target || opts.t || 'ES5',
		ignoreFilesPattern: opts.ignoreFilesPattern || false
	};
...
BranchCompiler.prototype.compileBranch = function (branch, manifest, cmpout) {
	...
	if (self.compiler.filesChanged() || branch.__tsc.needsCompile) {
		...
		var tsopts = {
			...
			sourceMap: !!self.opts.generateSourceMaps,
			declaration: !!self.opts.generateDeclarations,
...
BranchCompiler.prototype.updateFile = function (file, cmpout) {
	file.string = this.compiler.getCompiledFile(file.filename);

	if (!file.string) {
		if (this.shouldIgnoreFile(file.path, file.branch))
			return true;
		
		if (!branch.__tsc.error)
		...
		if (this.opts.generateSourceMaps)
			file.sourceMap = this.compiler.getSourceMap(file.filename);
